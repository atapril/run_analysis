...
for (i in 1:6){
        DataExtract$Activities <- gsub(activity_labels$V1[i], activity_labels$V2[i], DataExtract$Activities, ignore.case = TRUE)
}
# 4. Appropriately labels the data set with descriptive variable names
Name <- names(DataExtract)
...
names(DataExtract) <- Name

#5 From Date set in #4, create a tidy data set with the average of each variable for each activity and each subject.
TidyDataSet <- aggregate(DataExtract[,3:81], list( Activities = DataExtract$Activities, Subject=DataExtract$Subject),mean)
